...
module.exports = class PgPersistence {
  ...
  async completeAllTodos(todoListId) {
    const COMPLETE_ALL_QUERY = "UPDATE todos SET done = true" +
    "  WHERE todolist_id = $1 AND username = $2";

    let result = await dbQuery(COMPLETE_ALL_QUERY, todoListId, this.username);
  ...
  async createTodo(todoListId, title) {
    const CREATE_TODO_QUERY = "INSERT INTO todos (title, done, todolist_id, username) VALUES ($1, FALSE, $2, $3)";

    let result = await dbQuery(CREATE_TODO_QUERY, title, todoListId, this.username);
    return result.rowCount > 0;
  }
  ...
  async existsTodoListTitle(title) {
    const TODOLIST_EXISTS = "SELECT null FROM todolists WHERE title = $1 and username = $2";
  ...
  async loadTodoList(todoListId) {
    ...
    let result =  await dbQuery(FIND_TODOLIST, todoListId, this.username);

    let todoList = result.rows[0];
    todoList.todos = [];

    let todos = await dbQuery(FIND_TODOS, todoListId, this.username);
    for (let index = 0; index < todos.rows.length; ++index) {
      todoList.todos.push(todos.rows[index]);
    }
    return todoList;
  ...
  async setTodoListTitle(todoListId, title) {
    const UPDATE_TODO_LIST = "UPDATE todolists SET title = $1 WHERE id = $2 AND username = $3";

    let result = await dbQuery(UPDATE_TODO_LIST, title, todoListId, this.username);
    return result.rowCount > 0;
  }
